...
- **Exact Numerics & Rich Types** — `numeric` decodes to a `number` when a double holds it exactly and to a `Numeric` otherwise, never silently losing digits. Geometric values (`point`, `box`, `circle`, `line`, `path`, `polygon`), `interval`, and the range/multirange family each decode to a class of their own that round-trips through its own type, plus native decoding for network types (`inet`/`cidr`/`macaddr`), bit strings, `tsvector`/`tsquery`/`jsonpath`, and the system-column types (`tid`, `xid`/`xid8`/`cid`, `pg_lsn`). `unknownTypesAsString` asks the server for text on anything still unregistered instead of handing back a `Buffer`. See [Data Types & Type Mapping](../querying/data-types.md).
- **Reported Connection Loss** — A backend killed by an administrator, a failover, or a network fault rejects the in-flight call and fires `Connection`/`Pool` events with a `ConnectionLostError` carrying `code: '08006'` and the backend's process id — told apart from an application-requested `close()` by that code, without an `instanceof` check. See [Error Handling](../reliability/error-handling.md#lost-connections).
- **Server Notices** — A `RAISE NOTICE` or PostgreSQL's own "table does not exist, skipping" arrives as a `'notice'` event on `Connection`/`Pool`, shaped like a `DatabaseError` but never rejecting the call it came from. See [Error Handling: Server Notices](../reliability/error-handling.md#server-notices).
...
How postgrejs compares to [`pg`](https://github.com/brianc/node-postgres) (node-postgres) and [`postgres`](https://github.com/porsager/postgres) (postgres.js) — versions compared: **postgrejs 3.8.0, pg 8.23.0, postgres.js 3.4.9**. ✅ built in · 🟡 partial or needs a separate package · ❌ not supported.
...
| LISTEN/NOTIFY | ✅ | 🟡 | ✅ |
| ***Data types*** | | | |
| Text encoders | 125 | generic | 14 |
| Text decoders | 125 | 44 | 12 |
| Binary encoders | 121 | ❌ | ❌ |
