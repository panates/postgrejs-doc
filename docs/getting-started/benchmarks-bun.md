...
This report runs the same suite as [Benchmarks](./benchmarks.md) under Bun instead of Node.js, and adds Bun's own built-in `Bun.sql` as a fourth client in the comparison. Kept separate rather than merged into the Node report, since GC instrumentation, cursor streaming, and binary-format control all differ enough between the two runtimes that a single table would conflate a library difference with a runtime difference.
...
These numbers are produced by `benchmark/` (run via `npm run bench`), comparing PostgreJS against `pg` (node-postgres) and `postgres` (postgres.js) on the same server, the same machine, and the same workload. See [benchmark/README.md](https://github.com/panates/postgrejs/blob/dev/benchmark/README.md) for how to reproduce them.
