...
| inferrable          | `boolean`                          | `true`  | Whether `determine()` may pick this type via `isType`. Set `false` when `isType` can genuinely match a value but no caller means that type when passing a plain one — the type still decodes its own columns as before, and is still reachable explicitly with `new BindParam(oid, value)` |
